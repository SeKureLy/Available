...
DATA = YAML.safe_load File.read('app/db/seeds/receipt_seeds.yml')

describe 'Test AIS Web API' do
  ...
  describe 'Handle receipts' do
    it 'HAPPY: should be able to get list of all receipts' do
      AIS::Receipt.new(DATA[0]).save
      AIS::Receipt.new(DATA[1]).save

      get 'api/v1/receipts'
      result = JSON.parse last_response.body
      _(result['receipt_ids'].count).must_equal 2
    end
    ...
    it 'HAPPY: should be able to create new receipts' do
      ...
      post 'api/v1/receipts', DATA[1].to_json, req_header
